...
//            ->where('web_id', $web_id)
    ...
    public function store(Request $request){
        ...
        if($c1 != 0 || $c2 != 0) {
            if ($c1) {
                $conv = Conversation::where('user1',Auth::user()->id)->where('user2',$request->user_id)->first();
                return redirect()->route('conversation.show', [$conv->id]);
            }
            if ($c2) {
                $conv = Conversation::where('user2',Auth::user()->id)->where('user1',$request->user_id)->first();
                return redirect()->route('conversation.show', [$conv->id]);
            }
        ...
        return redirect()->route('conversation.show', [$c->id]);
//        return redirect()->back();
    }
